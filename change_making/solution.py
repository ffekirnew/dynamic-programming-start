        ...
        def dp(target: int):
            ...
            if target not in cache:
                ...
                for coin in coins:
                    ...
                    if not optimal_change or len(optimal_change) > len(
                        possible_solution
                    ):
                        optimal_change = possible_solution

                cache[target] = optimal_change or []
    ...
    def bottom_up(self, coins: List[int], target: int) -> List[int]:
        ...
        for money_point in range(1, target + 1):
            for coin in coins:
                if money_point > coin:
                    change_using_coin = (
                        optimal_changes[coin] + optimal_changes[money_point - coin]
                    )

                    if money_point not in optimal_changes or len(
                        optimal_changes[money_point]
                    ) > len(change_using_coin):
